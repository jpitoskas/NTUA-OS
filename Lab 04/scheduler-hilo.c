	...
	for (i = 0; i < size; i++) 
	{
		...
		if ( elem.id == id ) 
		{
			...
			kill(elem.pid, SIGKILL);
	 		printf("\nMeeseeks %d with Meeseeks PID: %ld and Priority: %s just died. Cause of Death: Existence's Pain\n\n", id, (long)elem.pid, elem.pr);
		}
		else
		{
			enqueue(&q, &elem);
...
sched_create_task(char *executable)
{
	...
	if ( new_pid > 0 )
	{
		...
		task.pr = "LOW";
		enqueue(&q, &task);
		printf("\nHEY, I'm  new Mr Meeseeks %d LOOK AT ME and my Meeseeks PID is %ld and Priority: %s. My Purpose is to execute: %s \n\n", task.id, (long)task.pid, task.pr, task.exec);
	}		
	if ( new_pid == 0 ) {
		raise(SIGSTOP);
...
sigchld_handler(int signum)
{
	...
	for (;;) {
		...
		/* 	if we have HIGH priorities and the current is one of them we run the current 
			...
			if we only have LOW priorities (except from shell) then we run the current regardless of whether it is HIGH or LOW	*/
